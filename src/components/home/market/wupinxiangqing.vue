  ...
  <div class="box">
    <div class="box-container" id="1">
      ...
      <div v-show="rapid" class="nav-top-back">
        ...
        <p @click="jubao()">
          <img
            ...
            alt
          />
          <span style="position: relative;left: 0.04rem;bottom:0.06rem">举报</span>
        </p>
      </div>
      ...
      <div class="concat">
        <p>
          ￥
          <span style="font-size: 0.48rem">{{dataList.price}}</span>
        </p>
        <!-- <p>518特惠</p> -->
        <p>
          价格
      ...
      <div class="concat-top">
        ...
        <div v-show="qq == 1 ? true :false">
          <p>
            ...
            <van-icon
              ...
              name="checked"
              size="0.3rem"
              style="position: relative;top: 0.06rem;right: 0.1rem"
            />领券后，下单至少可减
            <span>{{jiann}}元</span>
            <span @click="onClickk">
              领券
              <van-icon
                name="arrow"
                style="width: 0.2rem;position: relative;right: 0.48rem;top: 0.04rem"
              />
            </span>
          </p>
          <p>店铺优惠券</p>
        ...
        <div
          style="position: relative;margin-top: 0.26rem"
          v-if="dataList.tag.length !=0 ? true:false"
        >
          服务
          <p class="zxc" style="position: relative;top: -0.5rem;left:0.5rem;">
            <span :key="index" v-for="(item,index) in dataList.tag" style>
              <van-icon
                ...
                name="passed"
                size="0.3rem"
                style="position: relative;top: 0.06rem;right: 0.11rem"
              />
              {{item}}
            </span>
          </p>
      ...
      <div class="concat-zhong" id="2">
        ...
        <p>
          ...
          <van-icon
            color="#777777"
            name="arrow"
            style="position: relative;top: 0.25rem;float: right"
          />
        </p>
      </div>
      <div class="concat-bottom" v-if="chang == 0 ? false :true">
        <p @click="sypj()">
          ...
          <span>
            查看全部
            <van-icon color="#F21D13" name="arrow" style="position: relative;top: 0.04rem" />
          </span>
        </p>
        ...
        <div class="concat-footer-div" @click="dianpu()">
          ...
          <div class="concat-footer-div-div">
            ...
            <van-rate
              ...
              color="#EF0600"
              readonly
              style="position: relative;top: 0.1rem;left: 0.05rem"
              v-model="dataList.star"
              void-color="#FFFFFF"
            />
            <p class="concat-footer-div-div-p">
              进店
              <van-icon
                color="#F21D13"
                name="arrow"
                style="position: relative;top: 0.05rem;right: 0.4rem"
              />
            </p>
          </div>
        </div>
        <div class="concat-footer-two">
          <div v-if="dataList.description==''?false:true">
            描述相符
            <span>{{dataList.description}}</span>
          </div>
          <div v-if="dataList.quality==''?false:true">
            质量满意
            <span>{{dataList.quality}}</span>
          </div>
          <div v-if="dataList.service==''?false:true">
            服务周到
            <span>{{dataList.service}}</span>
          </div>
        </div>
      </div>
      <div id="3"></div>
      ...
      <div class="nav-tj" id="4">
        <div class="nav-tj-top">
          <span>为你推荐</span>
        </div>
        <div class="nav-tj-bottom">
          <div
            class="nav-tj-bottom-div"
            v-for="(item,index) in dataList.tjgoods"
            :key="index"
            @click="xq(item.id)"
          >
      ...
      <div class="navbar" v-show="bar">
        ...
        <div class="navbar-div">
          ...
          <div>
            <div :style="styleObj1" @click="ping()">
              <a :style="styleObj2" href="#2">评价</a>
            </div>
          </div>
          <div>
            <div :style="styleObj3" @click="xian()">
              <a :style="styleObj4" href="#3">详情</a>
            </div>
          </div>
          <div>
            <div :style="styleObj5">
              <a :style="styleObj6" @click="jian()" href="#4">推荐</a>
            </div>
          </div>
        </div>
        <div>
        ...
        <div class="pop">
          <div class="pop-div">
            ...
            <div class="pop-div-div" v-for="(item,index) in youu" :key="index">
              <img
                v-show="item.is_expire == 0 ? true:false"
                src="../../../assets/youhuioquan.png"
                alt
              />
              <img
                v-show="item.is_expire == 1 ? true:false"
                src="../../../assets/youhuiquanyilingquu.png"
                alt
              />
              <p>
                <span>￥</span>
                ...
                <span
                  v-show="item.is_expire == 0 ? true:false"
                >满{{item.min_price}}立减</span>
                <span v-show="item.is_expire == 1 ? true:false">满{{item.min_price}}立减</span>
              </p>
