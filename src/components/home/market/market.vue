        ...
        <div v-show="rapid" class="nav-top-back">
          ...
          <p>
            <img
              ...
              alt
            />
            <span style="position: relative;left: -0.02rem">分享</span>
          </p>
        </div>
      </div>
      <!-- 内容-->
      <div class="concat">
        <div class="concat-left">
          <ul>
            <li
              :key="index"
              :style=" back == index? backg :backgg "
              @click="qie(index,item.id)"
              v-for="(item ,index) in list"
            >
              <img :src="phh == index ? pho : ph" alt />
              <span>{{item.name}}</span>
            </li>
          </ul>
        </div>
        <div class="concat-right">
          ...
          <div
            @click="xian(item.sid)"
            class="concat-right-concat"
            v-for="(item,index) in bottom"
            :key="index"
          >
            <div>
              <img alt :src="item.shoplogo" />
            </div>
