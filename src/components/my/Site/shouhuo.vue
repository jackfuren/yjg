    ...
    <div v-show="show">
      <div class="concat" v-for="(item,index) in dataList" :key="index">
        <div @click="bj(index)">
          <p>
            {{item.consignee}}
            <span class="concat-s">{{item.phone}}</span>
            <span class="concat-ss" v-show="item.is_default == 1 ? true : false">&nbsp;默认&nbsp;</span>
          </p>
          <p>{{item.province}}{{item.city}}{{item.area}}{{item.address}}</p>
          <img alt src="../../../assets/dizhiliebiao_bianji.png" />
        </div>
      </div>
    </div>
    <div v-show="show1">
      <div class="concat" v-for="(item,index) in dataList" :key="index">
        ...
        <van-checkbox-group class="a" v-model="store">
          <van-checkbox :name="item.id" checked-color="#EF0600"></van-checkbox>
        </van-checkbox-group>
      </div>
    </div>
